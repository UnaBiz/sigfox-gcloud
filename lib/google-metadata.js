...
function getProjectMetadata(req, authClient) {
  ...
  return new Promise((resolve, reject) =>
    ...
    .then((res) => {
      ...
      common.log(req, 'google-metadata/getProjectMetadata', { result, device: req.device });
      return result;
    })
    .catch((error) => {
      common.log(req, 'google-metadata/getProjectMetadata', { error, device: req.device });
      throw error;
    });
}
...
function setProjectMetadata(req, authClient, metadata) {
  ...
  return new Promise((resolve, reject) =>
    compute.projects.setCommonInstanceMetadata(request, (err, res) =>
      (err ? reject(err) : resolve(res))))
    .then((result) => {
      common.log(req, 'google-metadata/setProjectMetadata', { result, metadata, device: req.device });
      return result;
    })
    .catch((error) => {
      common.log(req, 'google-metadata/setProjectMetadata', { error, metadata, device: req.device });
      throw error;
    });
}
