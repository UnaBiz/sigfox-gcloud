...
const common = require('../index');  // eslint-disable-next-line import/no-unresolved
const credentials = isGoogleCloud ? null : require('../google-credentials.json');

const scopes = ['https://www.googleapis.com/auth/cloud-platform'];
const compute = google.compute('v1');

function authorizeMetadata(req, scopes0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((authClient0) => {
      ...
      common.log(req, 'google-metadata/authorize', { result: 'OK', scopes2, device: req.device });
      return authClient;
    })
    .catch((error) => {
      common.log(req, 'google-metadata/authorize', { error, scopes2, device: req.device });
      throw error;
    });
}
...
function getMetadata(req, authClient) {
  ...
  return new Promise((resolve, reject) =>
    ...
    .then((res) => {
      ...
      const metadata = Object.assign({}, res.commonInstanceMetadata);
      if (metadata.kind) delete metadata.kind;
      const result = convertMetadata(req, metadata);
      common.log(req, 'google-metadata/getMetadata', { result, device: req.device });
      return result;
    })
    .catch((error) => {
      common.log(req, 'google-metadata/getMetadata', { error, device: req.device });
      throw error;
    });
}

function setMetadata(req, authClient, metadata) {
  ...
  return new Promise((resolve, reject) =>
    compute.projects.setCommonInstanceMetadata(request, (err, res) =>
      (err ? reject(err) : resolve(res))))
    .then((result) => {
      common.log(req, 'google-metadata/setMetadata', { result, metadata, device: req.device });
      return result;
    })
    .catch((error) => {
      common.log(req, 'google-metadata/setMetadata', { error, metadata, device: req.device });
      throw error;
    });
}
